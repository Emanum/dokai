...
def download_models():
    ...
    ignore = []
    snapshot_download(
        "naver-clova-ix/donut-base-finetuned-cord-v2", ignore_patterns=ignore
    )
    snapshot_download(
        "naver-clova-ix/donut-base-finetuned-cord-v2", ignore_patterns=ignore
    )
...
def main(prompt: str, init_image: bytes, guess_mode: bool = False,
    ...
    res = Model().inference.remote(prompt, init_image, guess_mode, model_name)
